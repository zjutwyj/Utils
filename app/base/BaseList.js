...
define('BaseList', ['jquery', 'underscore', 'backbone', 'BaseUtils', 'HandlebarsHelper'],
  function (require, exports, module) {
    ...
    BaseList = Backbone.View.extend({
       ...
       *        data: {}, // 附加的数据 此数据在BaseList BaseItem视图中可以获取  其中BaseList中为{{name}} ;
       *        BaseItem中为{{_options._data.name}} BaseCollecton为this.options.data BaseModel为this.model.get('_options').data
      ...
      _initCollection: function (collection, options) {
        ...
        debug(function(){
          if (!ctx._options.model){
            return 'BaseList中initialize参数中未添加模型类， 检查config.js是否addModule("model"), 或头部是否引入';
          }
        }, {type: 'error'});
      ...
      _addOne: function (model) {
        ...
        if (!this.filter && !this.composite &&
          ...
          itemView._setViewId(this.options.viewId || this._options.viewId);
          ...
          if (ctx.collection.url) {
            ...
            ctx.collection._load(ctx.collection, ctx, model).
              then(function (result) {
                if (ctx.options.instance) app.addData(ctx.options.instance, result.models);
                if (result.length === 0) {
                  console.log('从服务器上传回来的列表为空！检查XxxCollection中是否配置url参数， 点击' +
      ...
      _detail: function (options) {
        ...
        seajs.use(['dialog-plus'], function (dialog) {
          ...
          debug(function(){
            if (Est.isEmpty(ctx._options.detail)){
