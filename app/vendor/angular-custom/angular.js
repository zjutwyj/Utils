 ...
 * each of the subsequent controllers.   This prevents strange results in applications, where otherwise
  ...
  attr: function(element, name, value){
    ...
    if (BOOLEAN_ATTR[lowercasedName]) {
      if (isDefined(value)) {
        if (!!value) {
          element[name] = true;
            $(element).attr(name, lowercasedName);
        } else {
          element[name] = false;
            $(element).removeAttr(lowercasedName);
        }
      } else {
        return (element[name] ||
...
function createHttpBackend($browser, createXhr, $browserDefer, callbacks, rawDocument) {
  ...
  function jsonpReq(url, done) {
    ...
    // - fetches local controllers via XHR and evals them
    ...
    compile: function(element, attr) {
      if (attr.type == 'text/ng-template') {
        var templateUrl = attr.id,
            // IE is not consistent, in controllers we have to read .text but in other nodes we have to read .textContent
            text = element[0].text;
