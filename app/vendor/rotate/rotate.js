! function($) {
	...
	jQuery.fn.extend({
			...
			if (0 !== this.length && "undefined" != typeof a) {
				...
				for (var b = [], c = 0, d = this.length; d > c; c++) {
					...
					if (e.Wilq32 && e.Wilq32.PhotoEffect) e.Wilq32.PhotoEffect._handleRotation(a);
						...
						var f = $.extend(!0, {}, a),
							g = new Wilq32.PhotoEffect(e, f)._rootObj;
						b.push($(g))
					}
				}
				return b
		...
		return supportedCSS ? function(a, b) {
			a.Wilq32 = {
				PhotoEffect: this
			}, this._img = this._rootObj = this._eventObj = a, this._handleRotation(b)
		} : function(a, b) {
			if (this._img = a, this._rootObj = document.createElement("span"), this._rootObj.style.display = "inline-block", this._rootObj.Wilq32 = {
				PhotoEffect: this
			}, a.parentNode.insertBefore(this._rootObj, a), a.complete) this._Loader(b);
			else {
				var c = this;
		...
		_setupParameters: function(a) {
			...
			}, this._parameters.duration = a.duration || this._parameters.duration || 1e3, this._parameters.callback = a.callback || this._parameters.callback || function() {}, a.bind && a.bind != this._parameters.bind && this._BindEvents(a.bind)
		},
		...
		_BindEvents: function(a) {
			if (a && this._eventObj) {
				if (this._parameters.bind) {
					...
					for (var c in b) b.hasOwnProperty(c) && jQuery(this._eventObj).unbind(c, b[c])
				}
				this._parameters.bind = a;
				for (var c in a) a.hasOwnProperty(c) && jQuery(this._eventObj).bind(c, a[c])
			}
		},
		_Loader: function() {
		...
		_animate: function() {
			...
			if (b && !this._parameters.animatedGif) clearTimeout(this._timer);
				...
				if (this._canvas || this._vimage || this._img) {
					var c = this._parameters.easing(0, a - this._animateStartTime, this._animateStartAngle, this._parameters.animateTo - this._animateStartAngle, this._parameters.duration);
					this._rotate(~~(10 * c) / 10)
				}
		...
		_rotate: function() {
			var a = Math.PI / 180;
			return IE ? function(a) {
				this._angle = a, this._container.style.rotation = a % 360 + "deg"
			} : supportedCSS ? function(a) {
				this._angle = a, this._img.style[supportedCSS] = "rotate(" + a % 360 + "deg)"
			} : function(b) {
