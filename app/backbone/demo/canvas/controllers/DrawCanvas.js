define('DrawCanvas', ['Zepto', 'Canvas'], function(require, exports, module){

var Zepto = require('Zepto');
var Canvas = require('Canvas');

(function($){
    function DrawCanvas(el, opts) {
        this.conNode = el;
    ...
    DrawCanvas.prototype = {
        ...
        resizeCanvas_w: function(canvas, width, height, left, top) {
        	left = left || 0; top = top || 0;
            canvas.width = width, canvas.height = height, canvas.getContext("2d").clearRect(left, top, width, height), this.vail ? this.drawLottery() : this.drawMask()
        },
        ...
        drawLottery: function() {
            ...
            if ("image" == this.lotteryType) {
                ...
                this.width = this.width, this.height = this.height, this.resizeCanvas(this.background, this.width, this.height, this.left, this.top), this.backCtx.save(), this.backCtx.fillStyle = "#FFF", this.backCtx.fillRect(0, 0, this.width, this.height), this.backCtx.restore(), this.backCtx.save();
                var fontSize = 30;
        ...
        init: function(target, lotteryType) {
            ...
            /*window.addEventListener("resize", function() {
                ctx.width = document.documentElement.clientWidth, ctx.height = document.documentElement.clientHeight, ctx.resizeCanvas_w(ctx.mask, ctx.width, ctx.height, ctx.left, ctx.top)
            }, !1)*/
        }
    }
    ...
    if (window.jQuery || window.Zepto) {
        (function($){
            $.fn.drawcanvas = function(opts){
                return this.each(function(){
                    $(this).data('lottery', new DrawCanvas(this, opts).init());
                });
            }
