...
XIAOMI.namespace("app.analytics"),
    ...
    XIAOMI.app.footer = {
        ...
        conference0515: function() {
            function showCountLogo(ttime) {
                var $text,
                    remainTime = targetTime.getTime() - 1e3 * ttime,
                    remainDays = Math.floor(remainTime / 864e5),
                    remainHours = Math.floor(remainTime / 36e5) - 24 * remainDays,
                    remainMinutes = Math.floor(remainTime / 6e4) - 24 * remainDays * 60 - 60 * remainHours;
                if (remainDays > 0) $text = '<span class="time">' + remainDays + "</span>天";
                else if (remainHours > 0) $text = '<span class="time">' + remainHours + "</span>小时";
                else {
                    if (! (remainMinutes > 0)) return ! 1;
                    $text = '<span class="time">' + remainMinutes + "</span>分"
                }
                window.setTimeout(function() {
                        $('<div class="event-0515 clearfix"><a class="logo-0515" href="http://www.mi.com/index.php" title="小米官网">小米官网</a><a href="http://bbs.xiaomi.cn/thread-9712906-1-1.html" target="_blank" style="float: left; width: 140px;"><img src="http://img03.mifile.cn/webfile/images/g/2014/cn/index/515LogoIng.png" srcset="http://img03.mifile.cn/webfile/images/g/2014/cn/index/515LogoIng_2x.png 2x" width="95" height="69" /></a></div>').insertAfter(".site-logo .logo").fadeIn(1e3)
                    },
                    1e3)
            }
            var targetTime = new Date("2014/05/15 22:00:00");
            $.ajax({
                dataType: "script",
                url: "http://tp.hd.mi.com/gettimestamp",
                error: function() {
                    showCountLogo((new Date).getTime())
                },
                success: function() {
    ...
    XIAOMI.app.miniCart = {
        ...
        delGoods: function(gId) {
            ...
            $.ajax({
                type: "POST",
                url: XIAOMI.GLOBAL_CONFIG.orderSite + "/cart/delete/" + gId,
                dataType: "jsonp",
                jsonp: "jsonpcallback",
                cache: "false",
                success: function(data) {
        ...
        submit: function() {
            ...
            $("#J_searchForm").on("submit",
                function() {
                    ...
                    return "" !== val && (val = val.replace(/[\-_'=+|\\\/]/g, " "), window.location.href = XIAOMI.GLOBAL_CONFIG.wwwSite + "/search_" + encodeURIComponent(val)),
                        !1
                })
        }
    },
    ...
    XIAOMI.app.setLoginInfo = {
        ...
        updateDom: function() {
            ...
            var timeoutUserMenu,
                html = '欢迎您 <b class="user-name">' + displayName + '<span class="arrow"></span></b><span class="sep">|</span><a href="' + this.config.orderSite + "/user/order?r=" + randomStr + '">我的订单</a><span class="sep">|</span><a href="http://account.xiaomi.com/" target="_blank">我的小米账户</a>',
